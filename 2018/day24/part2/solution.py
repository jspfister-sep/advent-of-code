...
class War:
    def __init__(self, description_file):
        ...
        for line in description_file:
            ...
            if line:
                m = re.match('(.+):', line)
                if m:
                    self.armies_backup.append(Army(m.group(1)))
                else:
                    self.armies_backup[-1].add_group(Group(line))
        self.prepare_armies()

    def apply_boost(self, boost):
        for g in self.immune_system.groups:
    ...
    def fight(self):
        ...
        while self.armies[0].is_alive and self.armies[1].is_alive:
            ...
            if last_unit_count == self.armies[0].unit_count + self.armies[1].unit_count:
                return None 
            else:
                last_unit_count = self.armies[0].unit_count + self.armies[1].unit_count
        return self.armies[0] if self.armies[0].is_alive else self.armies[1]

    def fight_with_boost(self):
        enemy_with_most_health = max(self.infection.groups, key=lambda g: g.num_units * g.hit_points)
        min_boost = 0
        max_boost = enemy_with_most_health.num_units * enemy_with_most_health.hit_points 
        while min_boost < max_boost - 1:
            boost = int((min_boost + max_boost) / 2)
            self.prepare_armies()
            self.apply_boost(boost)
            victor = self.fight()
            if victor == self.immune_system:
                max_boost = boost
            else:
                min_boost = boost
        self.prepare_armies()
        self.apply_boost(min_boost + 1)
        return self.fight()
    ...
    def prepare_armies(self):
        self.armies = copy.deepcopy(self.armies_backup)
        self.immune_system = self.armies[0] if 'immune' in self.armies[0].name.lower() else self.armies[1]
        self.infection = self.armies[0] if 'infection' in self.armies[0].name.lower() else self.armies[1]
