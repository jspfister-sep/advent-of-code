...
class Explorer:
    ...
    def calculate_time_to_full_oxygen(self):
        self.location_map = self._explore()
        vertices = {} 
        visited_vertices = []
        for p in self.location_map.keys():
            v = self.Vertex(p)
            if self.location_map[p].is_goal:
                v.distance = 0
            vertices[p] = v
        ...
        return max([v.distance for v in visited_vertices])
...
def main():
    ...
    minutes = explorer.calculate_time_to_full_oxygen()
    print(f'It will take {minutes} minutes for the module to fill with oxygen')
