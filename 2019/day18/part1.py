...
class State:
    def __init__(self, location, held_keys=set()):
    ...
    def __repr__(self):
        return f'({self.location} {self.held_keys})'

    def __hash__(self):
        return hash(self.location) ^ hash(''.join(sorted(list(self.held_keys))))
    ...
    def _get_possible_next_states(self, current_state, current_num_steps, _param):
        ...
        for k in unheld_keys:
            if self._is_valid_key_choice(k, previous, current_state):
                ...
                possible_next_states.add((State(key_location, held_keys), num_steps))
